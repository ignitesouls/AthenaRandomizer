...
namespace Athena.Randomizer
{
    ...
    {
        private void _randomizeBase()
        {
            _paramsEditor = ParamsEditor.ReadFromRegulationPath(Constants.RegulationInBase);
            _randomizePerfumeBottlesBase();
            _paramsEditor.WriteToRegulationPath(Constants.RegulationOutBase);
        }

        private void _randomizePerfumeBottlesBase()
        {
